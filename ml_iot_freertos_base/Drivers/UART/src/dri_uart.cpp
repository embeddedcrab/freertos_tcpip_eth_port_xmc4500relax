 ...
 *
 */
long UART__::init( const eUART_Init_& init )
{
	/* Local Variables */
	...
	/* Return status */
	return (static_cast<long>(ret_status));
}
 ...
 *
 */
long UART__::set_baudrate( const unsigned long& speed )
{
	/* Local Arguments */
 ...
 *
 */
long UART__::transmit( const unsigned char * p_src, const unsigned long& length )
{
	/* Input argument validity */
	DRIVER_ASSERT( nullptr == p_src,
					static_cast<long>(eUART_Status_::UART_STATUS_INVALID_ARGUMENT) );
	DRIVER_ASSERT( 0U == length,
			static_cast<long>(eUART_Status_::UART_STATUS_INVALID_ARGUMENT) );
	DRIVER_ASSERT( 0U == control.state.init,
			static_cast<long>(eUART_Status_::UART_STATUS_FAILED) );

	/* Local Arguments */
	eUART_Status_ ret_status = eUART_Status_::UART_STATUS_FAILED;

	/* Check transmit busy status and perform operation */
	if( 0 == control.state.tx_busy )
	{
 ...
 *
 */
long UART__::receive( unsigned char * const p_dest, const unsigned long& length )
{
	/* Input argument validity */
 ...
 *
 */
long UART__::abort( const eUART_Abort_& tx_rx )
{
	/* Local Arguments */
	eUART_Status_ ret_status = eUART_Status_::UART_STATUS_FAILED;
...
void UART__::tx_isr_entry( UART__* p_handle )
{
	...
	else/* Data is fully sent, ensure it and complete the transfer */
	{
		...
		if( 1 == XMC_USIC_CH_TXFIFO_IsEmpty( p_handle->context.p_channel ) )
		{
			...
			{
				p_handle->fp_callback( p_handle->channel, nullptr, eUART_Event_::UART_EVENT_TX_COMPLETE );
			}
		}
	}
}
	...
	if( (1 == p_handle->control.state.rx_lock) && (1 == p_handle->control.state.rx_busy) )
	{
		...
		else
		{
			...
			else
			{
				...
				{
					p_handle->fp_callback( p_handle->channel, nullptr, eUART_Event_::UART_EVENT_RX_COMPLETE );
				}
			}
		}
	}
